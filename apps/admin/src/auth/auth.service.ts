...
export class AuthService {
  ...
  async applyAdmin(payload) {

    try {
      let { userId } = payload

      /* TODO */
      await this.userModel.findByIdAndUpdate(userId, { status: '1', type: '2' })
      // return new ResponseResult<UserModel>(200, {}, '获取用户信息成功')
  ...
  async auth(payload: AuthAdminDto) {
    try {
      let { userId, status } = payload
      // TODO
      // if (!(level in ['0', '1', '2'])) {
      //   return new ResponseResult(ResultCode.other, null, '用户类型参数错误')
      // }
      //
      // await this.userModel.findByIdAndUpdate(userId, { audit: '1', type: level })
      //
      // return new ResponseResult(ResultCode.success, null, '修改成功')

    } catch (err) {
      return new ResponseResult(ResultCode.other, null, err.message)
    }
  }

  async changeLevel(payload: ChangeLevelDto) {
    try {
      let { userId, level } = payload
      if (!(level in ['0', '1', '2'])) {
        return new ResponseResult(ResultCode.other, null, '用户类型参数错误')
      }
